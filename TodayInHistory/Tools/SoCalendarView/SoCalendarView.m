...
#define KK_WEEKSTITLE_HEiGHT (30*(self.frame.size.width/375.0))
...
-(void)addWeekLableToCalendarWatch {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        lable.font = [UIFont systemFontOfSize:12*(self.frame.size.width/375.0)];
...
-(void)addCalendarButton
{
    ...
    UIView *whiteBgView = [[UIView alloc]initWithFrame:CGRectMake(0, KK_WEEKSTITLE_HEiGHT, self.frame.size.width, self.frame.size.height-KK_WEEKSTITLE_HEiGHT)];
...
//        bgView.layer.masksToBounds = YES;
        ...
        UILabel *lunCalendar = [[UILabel alloc]initWithFrame:
                                CGRectMake(6*(self.frame.size.width/375.0),
                                           10*(self.frame.size.width/375.0),
                                           btnWidth-(12*(self.frame.size.width/375.0)),
                                           btnheigh-30*(self.frame.size.width/375.0))];
        ...
        lunCalendar.font = [UIFont systemFontOfSize:14*(self.frame.size.width/375.0)];
        // 节日用其他色标示
        ...
        UILabel *jBCalendar = [[UILabel alloc]initWithFrame:
                               CGRectMake(6*(self.frame.size.width/375.0) ,
                                          lunCalendar.frame.size.height + lunCalendar.frame.origin.y -10*(self.frame.size.width/375.0) ,
                                          btnWidth-(12*(self.frame.size.width/375.0)),
                                          30*(self.frame.size.width/375.0))];
        jBCalendar.font = [UIFont systemFontOfSize:8*(self.frame.size.width/375.0)];
...
-(void)btnAtionCalendar:(UIButton*)sender
{
    ...
    SoCalendar *calendar = dayArray[sender.tag -100];
    
    NSInteger currentDay = [[SoDateTools currentDate:@"YYYYMMdd"] integerValue];
    
    NSDate *data = [calendar nsDate];
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    formatter.dateFormat = @"YYYYMMdd";
    
    NSString *time = [formatter stringFromDate:data];
    NSInteger selectDay = [[formatter stringFromDate:data] integerValue];
    
    if (selectDay>currentDay) {
        [SVProgressHUD showInfoWithStatus:@"只能选择当天之前日期"];
...
- (void)leftHandleSwipe:(UISwipeGestureRecognizer *)gestureRecognizer {
    ...
    [fomatter setDateFormat:@"YYYYMM"];
    
    int currentMonth = [[fomatter stringFromDate:currentDate] intValue];
    
    if (currentMonth == (varMonth+varYear*100)) {
        [SVProgressHUD showInfoWithStatus:@"不能选择当月之后的月份"];
        return;
    }
    ...
    if (self.changeMonthBlcok) {
        self.changeMonthBlcok(varYear,varMonth);
    }
